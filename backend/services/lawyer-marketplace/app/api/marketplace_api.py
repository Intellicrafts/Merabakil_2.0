...
def _lawyer_public(
    ...
    *,
    ...
    return LawyerPublic(
        ...
        city=lawyer.city or "",
        pincode=getattr(lawyer, "pincode", None) or "",
...
async def verify_my_enrollment(
    body: MeVerifyRequest,
    ...
    repo = MarketplaceRepository(session)
    lawyer = await repo.get_lawyer_by_user(uuid.UUID(current.user_id))
    if not lawyer:
        raise HTTPException(status_code=404, detail="Lawyer profile not found")

    state = body.state or _detect_bar_council_state(body.enrollment_number)
    if not state:
        raise HTTPException(
            status_code=422,
    ...
    try:
        async with httpx.AsyncClient(timeout=90.0) as client:
            r = await client.post(
                ...
                json={"enrollment_number": body.enrollment_number, "state": state},
            )
        result = r.json()
    except Exception as exc:
