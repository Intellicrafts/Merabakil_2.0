...
export function DurationControlSlider({ appointment, disabled, onApply }: DurationControlSliderProps) {
  ...
  const applyDisabled = disabled || delta === 0;
  ...
  return (
    <div className="rounded-2xl border border-black/[0.06] bg-gradient-to-br from-slate-50/80 to-white/60 p-4 dark:border-white/10 dark:from-slate-950/30 dark:to-white/[0.02]">
      ...
      <div className="mt-4 space-y-2">
        <Slider min={0} max={60} step={5} value={delta} onValueChange={setDelta} disabled={disabled} />
        <div className="flex justify-between text-[10px] text-muted-foreground">
          <span>0 min</span>
          <span className="font-medium text-foreground">+{delta} min</span>
          <span>+60 min</span>
        </div>
      </div>
      <Button
        ...
        onClick={() => onApply(previewEnd.toISOString())}
      >
