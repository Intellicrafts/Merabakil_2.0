import type { RankedMarketplaceLawyer } from "@/lib/appointment-types";
...
export function toRankedLawyer(raw: RankedMarketplaceLawyer): RankedLawyer {
  return {
    id: raw.id,
    ...
    city: raw.city || raw.jurisdictions?.[0] || "India",
    jurisdictions: raw.jurisdictions ?? [],
    languages: raw.languages ?? [],
    years_experience: raw.years_experience ?? 0,
    rating: raw.rating ?? 0,
    review_count: raw.review_count ?? raw.rating_count ?? 0,
...
export function findBestMatch(prefs: MatchPreferences, catalog: LawyerProfile[]): MatchResult | null {
  let pool = [...catalog];
...
export function listLawyers(catalog: LawyerProfile[], filters: LawyerFilters = {}): RankedLawyer[] {
  const q = filters.query?.trim().toLowerCase() ?? "";
  let list = [...catalog];
...
export function getFeaturedLawyers(catalog: LawyerProfile[], limit = 3): RankedLawyer[] {
  return listLawyers(catalog, { sort: "rating", verifiedOnly: true }).slice(0, limit);
}

export function getLawyer(catalog: LawyerProfile[], id: string): LawyerProfile | null {
  return catalog.find((l) => l.id === id || l.slug === id) ?? null;
}
