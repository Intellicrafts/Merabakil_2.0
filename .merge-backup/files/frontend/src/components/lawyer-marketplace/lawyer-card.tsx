...
export function LawyerCard({ lawyer, index = 0, onView, onBook }: LawyerCardProps) {
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-x-0 top-0 h-px mp-shimmer-line opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      ...
      <div className="relative">
        <div className="mb-3 flex items-start justify-between gap-2.5">
          ...
          <div className="relative flex h-10 w-10 shrink-0 items-center justify-center">
            <div className="mp-match-ring absolute inset-0 rounded-full opacity-75" style={{ padding: 1.5 }}>
              <div className="h-full w-full rounded-full bg-white dark:bg-[hsl(220_14%_9%)]" />
            </div>
            <div className="relative text-center leading-none">
              <p className="text-[12px] font-bold tabular-nums tracking-tight">{lawyer.match_score}</p>
              <p className="text-[7px] font-medium uppercase tracking-wider text-muted-foreground">
                match
              </p>
            </div>
          </div>
      ...
      <div className="relative mt-3.5 flex gap-1.5 border-t border-black/[0.05] pt-3 dark:border-white/[0.06]">
        <button
          type="button"
          className="mp-btn-soft h-9 flex-1 rounded-xl text-[12px] font-semibold"
        ...
        <button
          type="button"
          className="mp-btn-primary h-9 flex-1 rounded-xl text-[12px] font-semibold"
