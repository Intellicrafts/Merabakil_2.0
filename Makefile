...
.PHONY: public
public: env ## Start full stack + Cloudflare Tunnel (one global HTTPS URL)
	bash scripts/stop_native.sh 2>/dev/null || true
	. .venv/bin/activate && LEGALOS_PUBLIC_MODE=1 python backend/scripts/run_public_stack.py

.PHONY: public-tunnel
public-tunnel: env ## Tunnel only — stack must already be running
	bash scripts/expose_public.sh tunnel

.PHONY: public-restore
public-restore: ## Restore local-only frontend API URLs after public tunnel
	bash scripts/expose_public.sh restore
